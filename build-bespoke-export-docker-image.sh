...
#   ./build-bespoke-export-docker-image.sh
...
readonly DOCKERFILE_PATH="docker/Dockerfile-bespoke-export"
# The hardware-specific architecture tag for this build.
readonly ARCH_TAG="zen3-sm86"
...
# We search for the line that starts with '__version__ = ' to get an exact match.
...
readonly FULL_VERSION_TAG="${DOCKER_USERNAME}/${IMAGE_NAME}:${ULTRALYTICS_VERSION}-torch${PYTORCH_VERSION}-${ARCH_TAG}-export"
# The general 'latest' tag for this specific export-focused build.
readonly LATEST_TAG="${DOCKER_USERNAME}/${IMAGE_NAME}:latest-export"
...
# We build once with the most specific tag. The --load flag is crucial to make the image
